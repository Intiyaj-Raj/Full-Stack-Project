...
const AdminProducts = () => {
  ...
  async function getAllProducts() {
    try {
      ...
      const response = await fetch(
        "https://full-stack-project-cw6d.onrender.com/api/getproduct",
      );
  ...
  async function handleDelete(id) {
    try {
      ...
      const response = await fetch(
        `https://full-stack-project-cw6d.onrender.com/api/productdelete/${id}`,
        {
          method: "DELETE",
        },
      );
          ...
          {product.map((item, index) => (
            ...
            >
              ...
              {item.productStatus === "In-Stock" ? (
                ...
                <p className="text-red-700 font-semibold mt-1">Out-Of-Stock</p>
              )}

              <div className="flex flex-col sm:flex-row justify-between mt-4">
                <Link
                  to={`/admin/edit-product/${item._id}`}
                  className="flex items-center gap-3 text-xl text-blue-500 hover:text-blue-800"
                >
                  <FaEdit />
                </Link>
                ...
                <Link
                  ...
                  className="flex items-center gap-3 text-xl text-red-500 hover:text-red-800"
                >
                  <AiFillDelete />
                </Link>
              </div>
